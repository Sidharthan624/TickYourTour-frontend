...
const [itineraries, setItinerary] = useState<Itinerary[]>([]);
...
const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
  try {
    ...
    if(title.trim().length < 3) {
      ...
      toast.error('Description should be atleast 5 characters long')
      return
    }
    ...
    itineraries.forEach(( itinerary, index) =>{
      formData.append(`itineraries[${index}]`, JSON.stringify(itinerary))
    } )
    photos.forEach((file) => {
            ...
            <form className="max-w-md mx-auto w-3/4" onSubmit={handleSubmit}>
                ...
                <div className="relative z-0 w-full mb-5 group">
                    <input
                        ...
                        name="group_size" // Update name for clarity
                        id="group_size"
                        value={groupSize}
                ...
                <div className="relative z-0 w-full mb-5 group">
                    <fieldset className="space-y-2">
                        <legend className="text-sm text-gray-900 font-semibold">Amenities (select all that apply):</legend>
