...
const Review = () => {
    ...
    useEffect(() => {
       const fetchPackageData = async () => {
        try {
            ...
            if(res?.data.success) {
                setPackageId(res.data.getPackage[0]._id)
            }
        } catch (error) {
            console.error(error);
    ...
    useEffect(() => {
        const fetchRatings = async () => {
            try {
                ...
                if(packageId) {
                    const res = await getRatings(packageId)
                if(res?.data.success)
    ...
    const handleReply = async (reviewId: string) => {
       try {
        navigate(`/provider/reply/${reviewId}`)
       } catch (error) {
        console.error(error);
                            ...
                            <div key={index} className="bg-white shadow-md rounded-lg overflow-hidden mb-8">
                                <div className="p-6">
                                    <div className="flex gap-6 items-center">
                                        ...
                                        />
                                        <div className="text">
                                            <p className="font-medium leading-8 text-gray-900 mb-0">{val.userId.name}</p>
